...
def perform_and_plot_tsne(data, labels, title, save_path):
    ...
    scatter = ax1.scatter(tsne_result[:, 0], tsne_result[:, 1], c=labels, cmap="viridis")
    ...
    scatter3d = ax2.scatter(tsne_result[:, 0],
                            tsne_result[:, 1],
                            tsne_result[:, 2],
    ...
    plt.colorbar(scatter3d, ax=ax2, label='Severity Level')
...
def perform_pca_and_tsne(feature_maps, prototypes, image_labels, prototype_labels, save_path):
    print("Performing separate PCA...")
...
def main(params):
    ...
    save_dir = Path(training_root_folder)
    save_dir.mkdir(exist_ok=True)
    ...
    perform_pca_and_tsne(feature_maps, prototypes, image_labels, prototype_labels, save_dir)
