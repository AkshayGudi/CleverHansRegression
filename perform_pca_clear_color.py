...
def extract_features_and_prototypes(model_path, dataloader):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(dataloader):
                ...
                i = i + 1
                print("extracting for batch - " + str(i))
...
def perform_and_plot_tsne(data, labels, title, save_path):
    ...
    for class_id in np.unique(labels):
        ...
        ax1.scatter(tsne_result[mask, 0], tsne_result[mask, 1], c=class_colors[class_id], label=class_names[class_id])
    ...
    for class_id in np.unique(labels):
        mask = labels == class_id
        ax2.scatter(tsne_result[mask, 0],tsne_result[mask, 1],tsne_result[mask, 2],
...
def perform_and_plot_pca(data, labels, title, save_path, class_colors=None):
    ...
    for class_id in np.unique(labels):
        mask = labels == class_id
        ax4.scatter(pca_result[mask, 0], pca_result[mask, 1], pca_result[mask, 2], 
                    c=class_colors[class_id], label=class_names[class_id])
...
def perform_pca_and_tsne(feature_maps, image_labels, prototypes, prototype_labels, save_path):
    ...
    perform_and_plot_tsne(feature_maps_reshaped, image_labels, "Feature Maps - TSNE", save_path / "tsne", custom_colors)
    perform_and_plot_pca(feature_maps_reshaped, image_labels, "Feature Maps - PCA", save_path / "pca", custom_colors)

    # Perform PCA on prototypes
    perform_and_plot_tsne(prototypes_reshaped, prototype_labels, "Prototypes - TSNE", save_path / "tsne", custom_colors)    
    perform_and_plot_pca(prototypes_reshaped, prototype_labels, "Prototypes - PCA", save_path / "pca", custom_colors)
...
def main(params):
    ...
    perform_pca_and_tsne(feature_maps, image_labels, prototypes, prototype_labels, save_dir)
